...
async def orchestrate_build(repo_path: Path):
    ...
    """
    # Use a clean dict for the payload to avoid Pydantic serialization quirks
    payload = {"path": str(repo_path)}

    async with httpx.AsyncClient(timeout=300.0) as client:
        try:
            print(f"🏥 Notifying Prep Service for {repo_path.name}...")
            # (Future Prep Service call goes here)

            print(f"📦 Notifying Debianizer at {DEBIANIZER_URL}...")
            # Explicitly sending as JSON with standard headers
            ...
            if response.status_code != 200:
                print(f"❌ Debianizer returned {response.status_code}: {response.text}")
            else:
                print(f"🚀 Debianization triggered for {repo_path.name}")

        except Exception as e:
            print(f"❌ Orchestration failed during handoff: {e}")

async def run_clone_and_orchestrate(request: CloneRequest, target_path: Path):
    """
    The heavy lifting happens here, away from the HTTP response loop.
    """
    try:
        ...
        # Success? Move to the next service
...
async def clone_repo(request: CloneRequest, background_tasks: BackgroundTasks):
    ...
    # Background the work so we can return '202 Accepted' immediately
    background_tasks.add_task(run_clone_and_orchestrate, request, target_path)

    return {"status": "accepted", "project": repo_name}

@app.get("/health")
async def health():
    return {"status": "online", "service": "cloner"}
